...
- The typeface fallback is now a message rather than a warning, is
  ...
  longer repeats it.
- Bug fix: the layout engine’s macro tables were never freed. They
  leaked at process exit, and
  [`unloadNamespace()`](https://rdrr.io/r/base/ns-load.html) left both
  them and the shared object in place. All are now released.
