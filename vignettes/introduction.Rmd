...
# Only Lete Sans Math is bundled with the package. Latin Modern, STIX,
# and TeX Gyre DejaVu are fetched on demand via download_math_font().
# Chunks demonstrating those fonts are evaluated only when they are
# registered (i.e. the reader has already downloaded them).
have_extra_fonts <- all(c("STIX Two Math",
                          "LatinModernMath-Regular",
                          "TeXGyreDejaVuMath-Regular") %in%
...
- Multiple math fonts: Lete Sans Math (bundled); Latin Modern Math,
  STIX Two Math, and TeX Gyre DejaVu Math fetched on demand
...
The package ships with **Lete Sans Math** as the default math font ---
it pairs naturally with R's default sans text. Three additional math
fonts are available on demand to keep the installed package small:

| Alias | Font | License |
|-------|------|---------|
| `"lm"` | Latin Modern Math | GUST Font License |
| `"stix"` | STIX Two Math | SIL Open Font License |
| `"dejavu"` | TeX Gyre DejaVu Math | Bitstream Vera + public-domain additions |

Fetch any of them once per user with `download_math_font()`:

```r
download_math_font("stix")   # downloads ~2.3 MB into user cache
download_math_font("lm")
download_math_font("dejavu")
```

Downloaded files are cached under
`tools::R_user_dir("gridmicrotex", "cache")` and re-registered
automatically on future package loads --- you only need to call
`download_math_font()` once per font per user.

```{r fonts-list}
available_math_fonts()
```

```{r fonts-default, fig.height=0.8, fig.width=2, out.width="50%", eval=have_extra_fonts}
...
```{r fonts, fig.height=1.5, fig.width=2, out.width="50%", eval=have_extra_fonts}
grid::grid.newpage()
...
for `load_font()` for instructions.
...
```{r font-pairing, fig.height = 1, fig.width = 6, out.width="70%", eval=have_extra_fonts}
grid::grid.newpage()
grid.latex(
