...
**These functions are a wrapper around `latex_grob()`** --- your markdown
is turned into LaTeX and drawn by the same engine. So everything
`latex_grob()` renders works here too, with the same fonts, the same
options and the same limits (see `vignette("getting-started")`), and this
...
`th`, `hr`, `img`, `math`, `footnote`, `div`, `span`. The properties
are colour, the font family, size, weight and slant, decorations, line
height, margins, left padding, alignment and the two borders. `?md_style` has the full
...
cells; a LaTeX `array` spans anything but is styled in LaTeX.
...
- **Footnotes** (`[^1]` with a `[^1]:` definition) put a superscript
  ...
  `markdown_box_grob()`; `markdown_grob()` shows the marker and drops
  the note.
