...
Markdown has no syntax for colour, underline, or super/subscript ---
CommonMark and GFM simply do not define one. The conformant way to ask
for them is the inline HTML that CommonMark already includes, so that is
what `gridmicrotex` reads:

| markdown | effect |
|---|---|
| `<span style="color:red">…</span>` | colour (any R colour name, `#rgb`, `#rrggbb`, `rgb()`) |
| `<span style="text-decoration:underline">…</span>` | underline |
| `<u>…</u>`, `<ins>…</ins>` | underline |
| `<s>…</s>`, `<del>…</del>` | strikethrough |
| `<sub>…</sub>`, `<sup>…</sup>` | sub/superscript |
| `<br>` | line break |
...
Markdown and `$math$` keep working inside a span, and spans nest. Use
`**` and `*` for bold and italic --- `<b>` and `<i>` are *not*
interpreted, since markdown already covers them. Anything else, tag or
CSS property, is dropped and the text inside it kept.
