...
`markdown_grob()` takes `hyphenate` directly, since it passes its extra
arguments to `latex_grob()`. For `markdown_box_grob()`, set it for the
document with `latex_options(hyphenate = TRUE)`.
