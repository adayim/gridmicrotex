...
tbl <- paste0(
  ...
  "\\rowcolor{#F6F8FA}",
  "\\multicolumn{3}{|c|}{\\textbf{Model comparison}}\\\\\\hline",
  "\\textbf{Model}&\\textbf{AIC}&\\textbf{R}^2\\\\\\hline",
  "\\text{linear}&214.3&0.71\\\\",
  "\\cellcolor{#DDF0DD}\\text{quadratic}",
  "&\\cellcolor{#DDF0DD}201.8&\\cellcolor{#DDF0DD}0.87\\\\\\hline",
  "\\end{array}$$"
)
...
doc <- paste0(
  "A [link](https://example.org) keeps its text, an ",
...
- **HTML tables** are not read. CommonMark treats `<table>` as a raw HTML
  block, which is dropped; only `<div>` is interpreted. GFM's own table
