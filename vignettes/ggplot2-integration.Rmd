...
Labels are written with `r"(...)"`, R's raw-string syntax, so backslashes
pass through untouched rather than needing to be doubled --- as in
`vignette("getting-started")`. Dollar-sign delimiters (`$...$`) are
stripped automatically, so `r"(\frac{a}{b})"` and `r"($\frac{a}{b}$)"`
