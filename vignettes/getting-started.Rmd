...
`\graphicspath{{figs/}}` adds a directory to search. `angle`, `trim` and
`clip` are recognised but not applied, and say so.
...
icon <- sprintf(r"(\includegraphics[width=0.5\textwidth]{%s})", fig)
grid.newpage()
...
Save the plot yourself, then reference the file:

```{r img-plot, eval = FALSE}
f <- tempfile(fileext = ".svg")
svglite::svglite(f, width = 3, height = 2)
print(p)          # whatever render call this object needs
dev.off()

grid.latex(sprintf(r"(\text{Trend }\includegraphics[width=2in]{%s})", f),
           input_mode = "math")
```

The package deliberately does not do this step for you. The correct render
call differs by object: a `ggsurvfit` assembles its risk table in its own
print method, so drawing it with `grid.draw()` would silently lose the
table; `ComplexHeatmap` needs `draw()`; a base plot needs replaying. Get
that wrong and the figure looks fine but is missing part of itself. You
also know the final output resolution, and the package does not.
