...
In practice that means output from `knitr::kable(format = "latex")` or
`xtable::print.xtable()` can be pasted in unedited:
...
grid::grid.newpage()
grid.latex(snippet, input_mode = "mixed", gp = grid::gpar(fontsize = 11))
```

Six things were handled there without any editing: the `%` comment and
the `table` float were dropped, `\centering` removed, `\caption` set
inline, `\toprule`/`\bottomrule` became thick rules, `\midrule` a plain
one, and `\emph` became italic. `input_mode = "mixed"` is the right mode
for pasted content --- it wraps the bare cell text for you, where
`"math"` would set it as math italics.
