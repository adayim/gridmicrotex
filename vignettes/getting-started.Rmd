...
- Base graphics --- `latex_options(device_math = TRUE)`, see
  `vignette("base-graphics")`
...
starts with, can typeset math too: turn on `latex_options(device_math = TRUE)`
and write `$...$` in any label, with no other change to your code.
...
The switch works at the graphics device, so it also reaches lattice, grid,
ggplot2 and other packages' plot methods. It is session-wide and has side
effects worth knowing before you rely on it. `vignette("base-graphics")`
covers when a label counts as math, a table drawn inside a base plot, how
to make room for a tall formula, and the full list of side effects.
