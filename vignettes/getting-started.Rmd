...
```{r basic, fig.height=0.9, fig.width=2.4, out.width="45%"}
...
- **`"typeface"`** (default) draws glyphs as native text, so PDF and SVG
  output stays selectable and searchable --- prose inside `\text{}` is
  emitted one word per element, so a viewer can find a word, though not a
  phrase spanning a space. Fonts are read straight from their OTF files,
...
`table` float were dropped, `\centering` removed, `\caption` set inline,
`\toprule`/`\bottomrule` became thick rules, `\midrule` a plain one, and
`\emph` became italic. `input_mode = "mixed"` is the right mode for pasted
content --- it wraps the bare cell text for you, where `"math"` would set
it as math italics.
