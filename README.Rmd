...
## Disclaimer
**A note on development**: This package was developed as a proof of concept exploring AI-assisted package creation. The architecture and specification were designed by me, and the core C++ integration (via [MicroTeX](https://github.com/NanoMichael/MicroTeX)) was facilitated largely by AI, with my review and oversight of the final output. I'm sharing it because it works and I hope others find it useful. Contributions, bug reports, and improvements from the community are very welcome.
...
```{r example-ggplot2-geom, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
library(ggplot2)
...
- Four bundled math fonts: Latin Modern Math, STIX Two Math, Lete Sans Math, TeX Gyre DejaVu Math
  - Latin Modern & STIX Two are serif --- pair with `gpar(fontfamily = "serif")`
  - Lete Sans & TeX Gyre DejaVu are sans-serif --- pair with `gpar(fontfamily = "sans")`
...
- LRU layout cache for repeated formulas
  (`latex_cache_info()` / `latex_cache_limit()` / `latex_cache_clear()`)
