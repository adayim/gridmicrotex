...
def read_metrics(glyph):
    '''
    Return a tuple to represents metrics, in (width, height, depth, xMin) order.
    xMin is the leftmost x-coordinate of the glyph outline in font design units
    (i.e., the glyph's left side bearing from the advance origin).
    '''
    # [xmin, ymin, xmax, ymax]
    # 0 is the baseline
    ...
    return (
        ...
        # xMin (may be negative for glyphs with ink left of the origin)
        int(bounding_box[0]),
    )
...
def shift_path_x(data, dx):
    '''Shift all absolute x-coordinates in the parsed path by `dx`.
    Relative commands (lowercase) encode deltas and must stay unchanged.
    '''
    if dx == 0:
        return
    for cmd in data:
...
def _ink_xmin_in_y_range(glyph, y_lo, y_hi):
    '''Leftmost ink x of `glyph` within the y-range [y_lo, y_hi].

    Scans the glyph's foreground layer and considers:
      - on-curve points whose y is inside the strip, and
      - straight-segment crossings with the strip's horizontal edges.
    Bezier curvature inside the strip is ignored; for OpenType assembly
    parts the connector region is by design a straight vertical stroke,
    so this is accurate in practice.

    Returns None if nothing is found (e.g. y-range is outside all ink).
    '''
    xs = []
    for contour in glyph.foreground:
        ...
        for i in range(n):
            p0 = pts[i]
            p1 = pts[(i + 1) % n]
            if p0.on_curve and y_lo <= p0.y <= y_hi:
                xs.append(p0.x)
...
def read_glyph_path(glyph, shift_x=0):
    # Create a temp file but close it immediately so Windows unlocks it
    fd, temp_path = tempfile.mkstemp(suffix='.svg')
    os.close(fd)

    # keep font coordinate-system, thus increase y from bottom to up
    glyph.export(temp_path, usetransform=True)
    ...
    if shift_x:
        shift_path_x(data, shift_x)
    mirror(data)
    return data


def read_glyph(
    glyph, is_math_font, parse_path, kern_subtable_names, scripts_subtable_names,
    path_shift_x=0,
):
    '''
    Return a tuple
    ...
    return (
        ...
        None if not parse_path else read_glyph_path(glyph, shift_x=path_shift_x),
        glyph.glyphname,
    )
...
def parse_otf(file_path, have_glyph_path, output_file_path, userstyle=""):
    ...
    # Pre-shift per-glyph path data so vertical-assembly parts are anchored
    # at their stroke edge (x = 0) rather than at each glyph's own advance
    # origin. MicroTeX stacks assembly parts at the same x, so parts with
    # different LSBs would otherwise produce visible horizontal steps where
    # the caps meet the extender (most obvious on serifed right brackets).
    assembly_shifts = (
        collect_vertical_assembly_shifts(font) if is_math_font and have_glyph_path
        else {}
    )
    ...
    for glyph_name in font:
        ...
        glyphs.append(read_glyph(
            ...
            path_shift_x=assembly_shifts.get(glyph_name, 0),
        ))
    ...
    with open(output_file_path, 'wb') as f:
        ...
        # current major version 6 (v6 adds per-glyph xMin to metrics)
